...
function drawOfficial() {
  const funnel=model.raw.funnel,rows=funnel?.articles||[],fields=[['impressions','インプレッション','回'],['pageviews','ページビュー','回'],['likes','スキ','件'],['comments','コメント','件']];
  ...
  $('#officialTable').innerHTML=table(['記事','インプレッション','ページビュー','スキ','コメント'],[...rows].sort((a,b)=>(M.numeric(b.pageviews)??-Infinity)-(M.numeric(a.pageviews)??-Infinity)).map(r=>`<tr><td class="article-cell">${byKey.has(r.key)?`<button class="article-title" data-detail="${esc(r.key)}" type="button">${esc(byKey.get(r.key).title)}</button>`:esc(r.key)}</td>${fields.map(([key])=>`<td class="num">${number(M.numeric(r[key]))}</td>`).join('')}</tr>`));
}
