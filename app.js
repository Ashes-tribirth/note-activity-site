...
function drawOfficial() {
  const funnel=model.raw.funnel,rows=funnel?.articles||[],fields=[['impressions','表示回数','回'],['pageviews','新PV','回'],['likes','スキ','件'],['comments','コメント','件']];
  $('#officialDate').textContent=funnel?.date?funnel.date+' の単日':'未取得';
  if(!rows.length){$('#officialCards').innerHTML=empty('新しい公式指標は未取得です。');$('#officialTable').innerHTML='';return;}
  ...
  $('#officialTable').innerHTML=table(['記事','表示回数','新PV','スキ','コメント'],[...rows].sort((a,b)=>(M.numeric(b.pageviews)??-Infinity)-(M.numeric(a.pageviews)??-Infinity)).map(r=>`<tr><td class="article-cell">${byKey.has(r.key)?`<button class="article-title" data-detail="${esc(r.key)}" type="button">${esc(byKey.get(r.key).title)}</button>`:esc(r.key)}</td>${fields.map(([key])=>`<td class="num">${number(M.numeric(r[key]))}</td>`).join('')}</tr>`));
}
